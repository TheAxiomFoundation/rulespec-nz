...
PARQUET_LAYERS_PATH = ROOT / "data" / "corpus" / "ingestion" / "local-parquet-layers.json"
...
def test_audit_manifest_has_required_top_level_fields() -> None:
    ...
    required = {
        "generated_at", "track_id", "jurisdiction", "purpose",
        "pipeline_sources", "local_verified_facts",
        "publication_state_claims", "source_precedence",
    }
...
def test_audit_manifest_records_all_nlp_pipeline_sources() -> None:
    ...
    assert NLP_SOURCE_IDS == set(sources), (
...
def test_local_verified_facts_have_expected_keys() -> None:
    ...
    expected = {"parquet_layer_status", "nz_legislation_extraction", "oracle_pinned_commits"}
...
def test_publication_state_claims_include_github_heads() -> None:
    ...
    for source_id in NLP_SOURCE_IDS:
        ...
        assert isinstance(gh["head_sha"], str) and len(gh["head_sha"]) >= 7, (
            f"{source_id} head_sha too short"
        )
...
def test_publication_state_claims_include_huggingface() -> None:
    ...
    for source_id in NLP_SOURCE_IDS:
        ...
        has_id = any(k in hf for k in ("repository", "dataset", "space", "state", "notes"))
...
def test_publication_state_claims_reflect_local_availability() -> None:
    ...
    for source_id, source_data in manifest.get("pipeline_sources", {}).items():
        ...
        assert local_available is not None, f"{source_id} missing local_extract_available"
...
def test_fallback_route_defined() -> None:
    ...
    assert "pco" in fallback.lower() or "cli" in fallback.lower() or "xml" in fallback.lower(), (
    ...
    for cluster in inventory.get("duplicate_clusters", []):
        ...
        for nlp_s in nlp_systems:
            ...
            for off_s in official_systems:
                ...
                assert off_idx < nlp_idx, (
                    f"{cluster['id']}: official must precede NLP"
                )
...
def test_audit_manifest_links_to_inventory() -> None:
    ...
    assert any(k in integration for k in ("modules_with_nlp_route", "notes", "status")), (
...
def test_local_parquet_layers_references_nlp_sources() -> None:
    layers = cast(dict[str, Any], json.loads(
