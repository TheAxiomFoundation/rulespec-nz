...
def test_provenance_blocker_ledger_matches_direct_rule_proofs() -> None:
    ...
    assert ledger["atomic_module_count"] == len(modules) == 41
    assert ledger["proof_atom_count"] == 1264
    assert ledger["resolved_proof_atom_count"] == 1227
    assert ledger["blocked_proof_atom_count"] == 37
    ...
    assert len(affected_rule_ids) == 29
    assert blocked_proof_atom_count == ledger["blocked_proof_atom_count"] == 37
