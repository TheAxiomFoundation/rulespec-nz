...
def test_provenance_blocker_ledger_matches_direct_rule_proofs() -> None:
    ...
    assert ledger["proof_atom_count"] == 1234
    assert ledger["resolved_proof_atom_count"] == 1184
    assert ledger["blocked_proof_atom_count"] == 50
    ...
    assert len(affected_rule_ids) == 36
    assert blocked_proof_atom_count == ledger["blocked_proof_atom_count"] == 50
