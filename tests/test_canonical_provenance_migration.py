...
def test_provenance_blocker_ledger_matches_direct_rule_proofs() -> None:
    ...
    assert ledger["resolved_proof_atom_count"] == 974
    assert ledger["blocked_proof_atom_count"] == 105
    ...
    assert len(affected_rule_ids) == 86
    assert blocked_proof_atom_count == ledger["blocked_proof_atom_count"] == 105
